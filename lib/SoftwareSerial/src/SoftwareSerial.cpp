...
inline void SoftwareSerial::setRXTX(bool input) {
    if (_half_duplex) {
        if (input) {
            if (active_in != this) {//Si on n'est pas en rx, on le devient
            ...
            // Serial.println("skipping RX mode");
            //}
        } else {
            if (active_in == this) { //Si on était en mode rx, on passe en tx
...
inline void SoftwareSerial::handleInterrupt(HardwareTimer *timer) {
    ...
    if (active_out) { //FIXME: cette cochonerie se met à 0 tout seul
        active_out->send();
    }
}
